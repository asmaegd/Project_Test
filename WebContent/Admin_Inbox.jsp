...
<%try{ int id_admin = Integer.parseInt(session.getAttribute("id_Admin").toString());
    ...
    <div id="wrapper">
      ...
      <!-- /.content-wrapper -->
		...
		ArrayList<CommentairesA> liste_msg = f.getMessage();
		
		for (int i = 0; i < liste_msg.size(); i++) {
			%> <tr><td><%= liste_msg.get(i).getId_commande() %> </td>
			<td><%= liste_msg.get(i).getId_cl() %> </td>
			<td><%= liste_msg.get(i).getCommantaire() %> </td>
			</tr>
		<% }
      %>	
